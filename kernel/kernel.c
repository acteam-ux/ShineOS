...
void execute_command(char *input) {
    if (compare_string(input, "EXIT") == 0) {
        ...
        print_string("ShineOS, the new perfect os. the commands are HELP, HELLO and EXIT(for now!)\n> ");
    }
    else if (compare_string(input, "GUI") == 0){
        print_string("graphics is not supported for now, but it will arrive soon. you can type HELP to see how you can use this command line interface.\n> ");
    }
    else if (compare_string(input, "HELLO") == 0) {
        print_string("Hi! what do you want to do?\n> ");
    }
